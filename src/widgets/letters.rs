...
impl Word {
    pub fn new(word: String, starting_x: f64, scale: u8) -> Self {
        Word { word, starting_x, scale }
    }
}
...
impl Shape for Word {
    fn draw(&self, painter: &mut Painter) {
        for (i, letter) in self.word.chars().enumerate() {
            let mut letter = Letter::new_letter(letter, self.starting_x + (i as f64) * 7.0 * self.scale as f64);
            letter.scale = self.scale;
            letter.draw(painter);
        }
    }
}